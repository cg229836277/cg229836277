...
```
- 第一步先把数据方法哦Daily所属的文件中，也就是放到内存中。
...
通过调用`UsageStatsXml.write`方法执行写xml操作：
> UsageStatsXml
...
```
到这里我们发现包名，时长，使用次数，mLastEvent对应的是前台或后台事件，是int类型，前台为1，后台为2，一天的结束时间事件为3。

### 3.5 项目问题复盘
- 结合源码分析问题

基本上现在Android比较新的版本中，都将应用使用详情数据写到磁盘了，但是在Android 9.0以下的版本中没有将应用使用次数写到磁盘。另外还要面临延迟20分钟写磁盘的操作，如果每次都从磁盘取数据，在Android 9.0以下的版本中读取的的次数一定是不准确的。
...
```
到后面，写入的数据颗粒度越来越小，比如应用可见时长，前台服务到前台的时长等都被写入磁盘。这是因为后面Android在设置中也做了应用使用详情功能，如果这些数据不写入的话，数据会有出入。
